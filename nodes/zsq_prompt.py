...
from ..config import SCRIPT_DIR,PROMPT_STYLES_DIR,FOOOCUS_STYLES_DIR,PROMPT_NUM,AGE_START,AGE_END
...
def get_prompt_from_fooocus(values=None, positive='', batch_start=0, batch_length=100):
    prompt_positive=[]
    prompt_negative=[]
    prompt_name = []    
    if values is not None:
        max_lenth = len(values) 
        if batch_start > max_lenth and batch_length < 0:
            batch_start = 0    
        ...
        prompt_positive.append(positive)
...
def get_prompt_from_key(key,required):    
    ...
    list_value = list_value_dict.split("-")[-1]   
    print(list_value)
    return list_value
...
def get_propmt_style(self,required,Random,Random_Flag):
    ...
    res1 = ""
    if gender!= "-":
            res1= res1 + f",(a ({str(age)}-year-old:1.5) {gender})"
    if not Random_Flag:
...
class PortraitStyler:
    ...
    def INPUT_TYPES(self):
        ...
        fooocus = ["nothing"]
        fooocus_dir = FOOOCUS_STYLES_DIR
        for file_name in os.listdir(fooocus_dir):
            file = os.path.join(fooocus_dir, file_name)
            if os.path.isfile(file) and file_name.endswith(".json"):
                fooocus.append(file_name.split(".")[0])
               
        types = {
            "required":{
                        "GO_positive": ("STRING", {"default": "", "multiline": True}),
                        "GO_negative": ("STRING", {"default": "", "multiline": True}),
                        "GO_fooocus": (fooocus, {"default": "fooocus_styles"}),
                        "GO_Random": (["NO", "ONLY", "EXCEPT", "NOSELECT","SELECTED", "ALL"], {"default": "NO"}), 
                        "GO_ImageNum": ("INT", {"default": 1, "step": 1, "min": 1, "max": PROMPT_NUM, "display": "slider"}),                   
                        "GO_Random_Item":((style_names),),
                        "GO_gender": (["-", "Man", "Woman"], {"default": "-"}),
                        "GO_age": ("INT", {"default": 25, "step": 1, "min": AGE_START, "max": AGE_END, "display": "slider"}),
                        **widgets,},
            }
    ...
    def run(self,**required):
        ...
        prompts = []
        if Random != "NO": 
            fooocus = required["GO_fooocus"]
            values = []
            if fooocus != "nothing":                
                file = os.path.join(FOOOCUS_STYLES_DIR, fooocus + '.json')
                fooocus_json = read_json_file(file)
                for d in fooocus_json:
                    values.append(d)
            ...
            for i in range(ImageNum):
                ...
                if max_length > 0:
                    batch_start = rd.randint(0, max_length - 1)
                    _name,_positive, _negative = get_prompt_from_fooocus(values=values, positive=positive, batch_start=batch_start, batch_length=1)
                    res1 = _positive[0] + "," + res1
                else:
                    res1 = positive + "," + res1
                prompts.append(res1)                
        else:
...
class stylesPromptSelector:
    ...
    def INPUT_TYPES(s):
        ...
        return {
            "required": {
               "styles": (fooocus, {"default": "fooocus_styles"}),
            },
            "optional": {
