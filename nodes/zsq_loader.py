...
class checkpoint_sampler_tripleclip(checkpoint_sampler):
    @classmethod
    def INPUT_TYPES(cls):
    ...
    def run(self, resolution,**required,):
        ...
        try:
            ckpt_name = required.get("ckpt_name", "")
            clip_name1 = required.get("clip_name1", "")
            clip_name2 = required.get("clip_name2", "")
            clip_name3 = required.get("clip_name3", "")
        ...
        if self.model is None:
            print("Loading models:" + ckpt_name)
            self.model, tmpclip, self.vae = load_checkpoint_Simple(ckpt_name)
        
        self.clip = comfy.sd.load_clip(ckpt_paths=[clip_path1, clip_path2, clip_path3], embedding_directory=folder_paths.get_folder_paths("embeddings"))
...
class checkpoint_sampler_dualclip(checkpoint_sampler):
    @classmethod
    def INPUT_TYPES(cls):
    ...
    def run(self, resolution,**required,):
        ...
        if self.model is None:
            print("Loading models:" + ckpt_name)
            self.model, _, self.vae = load_checkpoint_Simple(ckpt_name)

        clip_name = clip_name1 + "," + clip_name2
        print(f"Loading clip: {clip_name}")
        if types == "sdxl" and ("clip_l" in clip_name) and ("clip_g" in clip_name):            
            clip_type = comfy.sd.CLIPType.STABLE_DIFFUSION
        elif types == "sd3" and ("clip_l" in clip_name) and ("t5" in clip_name):
            clip_type = comfy.sd.CLIPType.SD3
        elif types == "flux" and ("clip_l" in clip_name) and ("t5" in clip_name):
            clip_type = comfy.sd.CLIPType.FLUX
        else:
            raise ValueError("sdxl or kolors: clip-l, clip-g | sd3: clip-l, clip-g / clip-l, t5 / clip-g, t5 | flux: clip-l, t5")
        
        self.clip = comfy.sd.load_clip(ckpt_paths=[clip_path1, clip_path2], embedding_directory=folder_paths.get_folder_paths("embeddings"), clip_type=clip_type)
...
class checkpoint_sampler_unet(checkpoint_sampler):
    @classmethod
    def INPUT_TYPES(cls):
    ...
    def run(self, resolution,**required,):
        ...
        try:
            ...
            weight_dtype = required.get("weight_dtype", "default")
            ...
            optional_controlnet_stack = required.get("optional_controlnet_stack", None)
            #get ckpt_name from {"content": ckpt_name}
            #unet_name = get_content_from_key(unet_name)
        except Exception as e:
            print(e)
            return empty_latent()
        ...
        # Clean models from loaded_objects
        if self.model_name != unet_name:
            self.model = None
            self.model_name = unet_name
        # Load models
        if self.model is None:
            model_options = {}
            ...
            print("Loading models:" + unet_path)
        ...
        if types == "sdxl" and ("clip_l" in clip_name) and ("clip_g" in clip_name):            
            clip_type = comfy.sd.CLIPType.STABLE_DIFFUSION
        elif types == "kolors" and ("clip_l" in clip_name) and ("clip_g" in clip_name):
            clip_type = comfy.sd.CLIPType.STABLE_DIFFUSION
        elif types == "sd3" and ("clip_l" in clip_name) and ("t5" in clip_name):
            clip_type = comfy.sd.CLIPType.SD3
        elif types == "flux" and ("clip_l" in clip_name) and ("t5" in clip_name):
