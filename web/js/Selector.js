...
async function getStylesList(name){
    if(styles_list_cache[name]) return styles_list_cache[name]
       ...
       const resp = await api.fetchApi(`/prompt/styles?name=${name}`);
       if (resp.status === 200) {
...
async function getStylesImage(name, styles_name){
    ...
    if(styles_image_cache[styles_name][name]) return styles_image_cache[styles_name][name]
       ...
       const resp = await api.fetchApi(`/prompt/styles/image?name=${name}&styles_name=${styles_name}`);
       if (resp.status === 200) {
           ...
           const url = `/prompt/styles/image?name=${name}&styles_name=${styles_name}`
            ...
            "label.prompt-styles-tag",
            {
                ...
                $: (el) => {
                    ...
                    el.children[0].onclick = () => {                        
                        el.classList.toggle("prompt-styles-tag-selected");
                    };
                    el.onmousemove = (e) => {
...
app.registerExtension({
    name: 'comfy.zsq.styleSelector',
    async beforeRegisterNodeDef(nodeType, nodeData, app) {

        if(nodeData.name == 'stylesSelector'){
            // 创建时
            ...
            nodeType.prototype.onNodeCreated = function() {
                ...
                const list = $el("ul.prompt-styles-list",[]);
                ...
                //list.classList.add('prompt-styles-list')
                let styles_values = ''
                this.setProperty("values", [])

                let selector = this.addDOMWidget('select_styles',"btn",$el('div.prompt-styles',[$el('div.tools', [
                        $el('button.delete',{
                            textContent: $t('Empty All'),
                            style:{"height":"30px","width":"80px","border-radius":"8px","margin-right":"10px","color":"var(--e)",
                                "border":"2px solid var(--border-color)","font-size":"11px","background":"var(--comfy-input-bg)",
                                "cursor":"pointer","border-radius": "8px"},
                            onclick:()=>{
                                selector.element.children[0].querySelectorAll(".search").forEach(el=>{
                                    el.value = ''
                                })
                                selector.element.children[1].querySelectorAll(".prompt-styles-tag-selected").forEach(el => {
                                    el.classList.remove("prompt-styles-tag-selected");
                                    el.children[0].checked = false
                                })
                                selector.element.children[1].querySelectorAll(".prompt-styles-tag").forEach(el => {
                        ...
                        $el('textarea.search',{
                            ...
                            oninput:(e)=>{                                
                                ...
                                selector.element.children[1].querySelectorAll(".prompt-styles-tag").forEach(el => {
                                    ...
                                    if(name.indexOf(lower_value) != -1 || tag.indexOf(lower_value) != -1  || el.classList.value.indexOf("prompt-styles-tag-selected")!=-1){
                                        el.classList.remove('hide')
                                    }
                                    else{
                                ...
                                if(styles_list_cache[styles_values]){
                                    ...
                                    selector.element.children[1].querySelectorAll(".prompt-styles-tag").forEach(el => {
                                        if (this.properties["values"].includes(el.dataset.tag)) {
                                            el.classList.add("prompt-styles-tag-selected");
                                        }
                ...
                Object.defineProperty(selector, "value", {
                    ...
                    get: () => {
                        selector.element.children[1].querySelectorAll(".prompt-styles-tag").forEach(el => {
                            if(el.classList.value.indexOf("prompt-styles-tag-selected")>=0){
                                if(!this.properties["values"].includes(el.dataset.tag)){
                ...
                style_lists_dom.addEventListener('mouseenter', function (e) {
                    let value = ''
                    style_lists_dom.querySelectorAll(".prompt-styles-tag-selected").forEach(el=> value+=el.dataset.tag)
                    old_values = value
                })
                style_lists_dom.addEventListener('mouseleave', function (e) {
                    ...
                    if(old_values != new_values){
                        ...
                        style_lists_dom.querySelectorAll(".prompt-styles-tag").forEach(el => {
